...
function M.build_frontmatter(data)
  ...
  local field_order = {
    "timestamp", "id", "aliases", "capture_id",
    "tags", "sources", "modalities", "context",
    "location", "metadata", "processing_status",
...
function M.move_to_destination(source_path, destination_folder)
  ...
  if utils.path_exists(destination_path) then
    ...
    repeat
      destination_path = string.format("%s/%s_%d.%s",
        destination_folder, base_name, counter, extension)
      counter = counter + 1
...
function M.merge_into_note(source_path, target_path)
  ...
  local source_fm_str, source_body = utils.extract_frontmatter(source_content,
    config.patterns.frontmatter_delimiters)
  local source_fm = {}
  ...
  local target_fm_str, target_body = utils.extract_frontmatter(target_content,
    config.patterns.frontmatter_delimiters)
  local target_fm = {}
