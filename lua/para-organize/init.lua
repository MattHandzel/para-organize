...
function M._setup_plug_mappings()
  ...
  vim.keymap.set("n", "<Plug>(ParaOrganizeStart)", function()
    M.start()
  end, { silent = true, desc = "Start para-organize session" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeStop)", function()
    M.stop()
  end, { silent = true, desc = "Stop para-organize session" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeReindex)", function()
    M.reindex()
  end, { silent = true, desc = "Reindex notes" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeSearch)", function()
    M.search()
  end, { silent = true, desc = "Search notes" })

  -- Session-specific <Plug> mappings (will be active in UI buffers)
  vim.keymap.set("n", "<Plug>(ParaOrganizeAccept)", function()
    M.accept()
  end, { silent = true, desc = "Accept suggestion" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeMerge)", function()
    M.merge()
  end, { silent = true, desc = "Merge with existing note" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeArchive)", function()
    M.archive()
  end, { silent = true, desc = "Archive current capture" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeNext)", function()
    M.next()
  end, { silent = true, desc = "Next capture" })
  vim.keymap.set("n", "<Plug>(ParaOrganizePrev)", function()
    M.prev()
  end, { silent = true, desc = "Previous capture" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeSkip)", function()
    M.skip()
  end, { silent = true, desc = "Skip current capture" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeNewProject)", function()
    M.new_folder("projects")
  end, { silent = true, desc = "Create new project" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeNewArea)", function()
    M.new_folder("areas")
  end, { silent = true, desc = "Create new area" })
  vim.keymap.set("n", "<Plug>(ParaOrganizeNewResource)", function()
    M.new_folder("resources")
  end, { silent = true, desc = "Create new resource" })
end
...
function M.start(filters)
  if not initialized then
    vim.notify(
      "para-organize not initialized. Run require('para-organize').setup() first",
...
function M.stop()
  ...
  utils.log("INFO", "Session ended. Processed %d/%d captures", processed_count, #session.captures)
...
function M.reindex()
  ...
  indexer.full_reindex(function(stats)
    ...
    vim.notify(
      string.format("Indexed %d notes in %.2fs", stats.total, stats.duration),
      vim.log.levels.INFO
    )
  end)
end
...
function M.new_folder(folder_type, name)
  ...
  vim.notify(string.format("Created %s: %s", folder_type:sub(1, -2), name), vim.log.levels.INFO)
...
function M.debug_info()
  ...
  if stats.total == 0 and #captures > 0 then
    ...
    utils.log("INFO", "*** Running reindex now... ***")
    indexer.full_reindex(function(new_stats)
  ...
  if session then
    utils.log("INFO", "\n========== Session Info ==========")
    utils.log("INFO", "Active session: yes")
    utils.log("INFO", "Total captures: %d", #session.captures)
    utils.log("INFO", "Current index: %d", session.current_index)
  else
    utils.log("INFO", "\nNo active session")
  end
