...
- Not rebindable, and deliberately not nine rows in `keymaps.buffer`: there is no sane way to rebind "the digits", so the honest surface is one boolean. `actions.keymap_table()` emits **one synthetic help row** (`1-9 / accept Nth suggestion / organize`) when the flag is on and zero rows when off, so the generated `?` overlay (03 §2: help "must be generated from the actual keymap table") stays truthful. The nine digits are nevertheless nine separate `CORE_KEYMAPS` rows (§6 collision proof, item 7) — the help surface may collapse them, the collision surface may not. ⚠ **The two surfaces therefore disagree by nine rows on purpose, and 14 §10.7's drift gate names that exemption by name**: `1`–`9` are subtracted from both sides of the `CORE_KEYMAPS` ≡ `keymap_table()` equality, and any other difference still fails. The alternative — emitting nine rows from `keymap_table()` so the sets match — is rejected here rather than left open: `keymap_table()` is where the closed `keymaps.buffer` name set is derived, so nine rows would publish nine rebindable names for one behaviour governed by one boolean, and a user who tried to set any of them would be configuring a key that does not exist.
...
### 3.8 Destination preview — `p` (`toggle_preview`, narrowed here)

03 §3 flags `toggle_preview` as a binding that "must actually be bound, or delete the option". This is what it is for. ⚠ **The row is not new, and an implementer diffing against the tree should not hesitate over it**: `p` → `toggle_preview` is bound in the current tree in **both** panes (`actions.lua:1869`) and is already reserved in the core's `CORE_KEYMAPS`. What 03 §1 and 08 §A34 list among the ~25 dead keys is the **original** plugin's `keymaps.buffer.toggle_preview` — documented there and never bound. So this document gives the binding a behaviour and **narrows** it (§6: organize pane only, view-scoped); it does not add it, and it is not grandfathered either. With preview on, the **selected** destination expands inline (virtual lines) to show: its doc 11 §3 description, its note count, and the `ui.organize.preview_notes` (default `5`) most-recently-modified notes inside it — *what already lives there, before committing*.
...
⚠ **Sectioning is the arbitrated one**, `section = subsystem` per 14 §4.2: `ui.capture.*` and `ui.organize.*` (doc 15), `ui.hint.*` and `ui.batch.*` (this document), `ui.undo.*` (17), `ui.teach.*` (18), plus the existing top-level `ui.layout` / `auto_move_to_new_folder` / `highlights` / `icons` / `float_opts` / `win_options` / `capture_pane_keymaps` / `close_on_complete` — the complete set of live `SCHEMA` leaves, the first two included because an enumeration that calls itself canonical and omits a working key deletes it by accident. **`ui.display.*` does not exist** — doc 15 owns that surface and deletes the section, and no document may add a key under it. **Ordering**: doc 15 lands the closed `ui.capture` / `ui.organize` records *first*; this document appends leaves to `ui.organize` and adds `ui.hint` / `ui.batch` afterwards.
...
Pane column: **both** = bound in the organize pane and, subject to `ui.capture_pane_keymaps`, in the capture pane; **organize** = organize pane only. Rows are added through `actions.register_key{ name, default, desc, panes, views, fn }` — never by editing `CORE_KEYS` directly (14 owns `actions.lua`). The scoping field is **`views`**, a list of view names; there is no `mode` field, because in Neovim "mode" means the Vim mode and the live table already scopes by view. ⚠ `views` is a **rename and a widening** of a field that exists today in the singular — `view = "integrate"` on the live `integrate_edit` row — not a field invented here; **14 §5 states that migration once**, and this document writes its rows in the post-migration shape. Every single-character row this document adds is additionally view-scoped — see "Neither pane is safe to shadow in", below.
...
7. **Metadata reservation — one patch, one owner.** The complete `CORE_KEYMAPS` reservation for docs 12/15/16/17/18 lands as **ONE patch owned by doc 14** (14 §5, where the whole 30-row patch is tabulated), which also carries the drift gate (`CORE_KEYMAPS`'s key set equals the set of non-metadata `lhs` values in `keymap_table()`, minus the nine digits it exempts by name). Doc 13 is **not** in that list — its `A` → `auto_organize` row is already in the table, as is `p` → `toggle_preview` (§3.8). This document **contributes** `f`, `F`, `.`, `P`, `x`, the **nine** digits as nine separate rows, `<leader>ba`, `<leader>bc`, `<leader>d`, `<leader>f`. The pre-existing gap — `CORE_KEYMAPS` today is missing `e`, `<leader>mi` and `<leader>mm` (doc 12's rows), so `[[metadata_fields]] keymap = "e"` is currently accepted and one of the two bindings loses silently — is fixed **in that patch**, not "in the same change" as this document's. The point of the reservation is unchanged: a colliding `metadata_fields` entry fails **config validation** and is reported by `:checkhealth`, never discovered by a keypress doing the wrong thing.
...
The grandfathered exceptions in the **capture** pane are the existing `s`/`a`/`m`/`r` rows, which 03 §3 binds in both panes and which `ui.capture_pane_keymaps` exists to control: `"core"` (spec parity, default) binds them, `"navigation"` binds only rows flagged `navigation = true`, `"none"` leaves the capture buffer untouched. ⚠ **`p` is not among them — and it is not new either.** `p` → `toggle_preview` is bound in the tree today in both panes (`actions.lua:1869`) and already reserved in `CORE_KEYMAPS`; the ~25 dead keys 03 §1 lists are the **original** plugin's, where `keymaps.buffer.toggle_preview` was documented and never bound. What §3.8 does is give it a behaviour and **narrow** it: `panes = {"organize"}` and view-scoped like the rest. The grandfather clause does not reach it, because that clause covers the four rows 03 §3 deliberately binds in both panes (`s`/`a`/`m`/`r`, controlled by `ui.capture_pane_keymaps`) — not every row that happens to be bound in both today. The shadowing rule therefore applies unqualified, and `p` shadows paste-after in exactly the buffer this section exists to protect. Of the new rows, only `<leader>f` is flagged `navigation = true`. **`ui.capture_pane_keymaps = "none"` must leave zero new bindings on the capture buffer** — pinned as an obligation, not assumed.
