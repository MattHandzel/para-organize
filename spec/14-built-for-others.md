...
**The canonical nvim `ui.*` sectioning** — one table so every later doc's key is mechanically checkable — is `ui.capture.*` and `ui.organize.*` (doc 15, extended by 16), `ui.hint.*` and `ui.batch.*` (16), `ui.undo.*` (17), `ui.teach.*` (18), alongside the existing top-level `ui.layout`, `ui.auto_move_to_new_folder`, `ui.highlights`, `ui.icons`, `ui.float_opts`, `ui.win_options`, `ui.capture_pane_keymaps`, `ui.close_on_complete`. ⚠ **The list is canonical, so it is also complete**: every one of those eight is a live leaf of `config.lua`'s `SCHEMA` today, including the two flat scalars `ui.layout` and `ui.auto_move_to_new_folder`, which an earlier draft of this enumeration dropped. Against a closed record an omission from a list that calls itself canonical reads as a deletion, and a deletion nobody decided is how a working key stops validating. **There is no `ui.display` section**: doc 15 deletes it and no doc may add a key under it (§4.4 records the moves, §5 the seam).
...
| Keymaps | `keymaps.buffer.<name>` by action name; `""` unbinds. The reservation list `CORE_KEYMAPS` (`config.py`) and the action table (`actions.lua`) are **owned by this doc**, because a constant five documents write conflicts five ways: 15–18 submit their rows as **one consolidated patch** — tabulated in full below, and it closes doc 12's three missing rows in the same change — and a new binding arrives through `actions.register_key` (§5's "Extra actions"), never by editing `CORE_KEYS` in place. §10.7 gates the two against each other | STABLE (the **name set** is the contract; adding names is a MINOR) |
...
| Highlight groups | `ui.highlights.*` **and** the `ParaOrganize*` default groups the plugin defines | STABLE — ✅ *discharged 2026-08-21.* `ui.HL_GROUPS` is the closed set of seven groups the plugin can emit (`ParaOrganizeSelected\|Header\|Reason\|Hint\|ScoreHigh\|ScoreMedium\|ScoreLow`); `ui.apply_highlights()` defines each with `nvim_set_hl(0, name, { default = true, link = … })` from `setup()`, and re-applies on `ColorScheme` because `:colorscheme` issues `:hi clear`. The `ui.highlights` DEFAULTS now name these groups rather than the stock ones, so the advertised hook is what actually renders; `default = true` means a colorscheme that defines any of them wins. Before this, `nvim_get_hl` returned an empty dict for all four named fallbacks — indistinguishable from a group that does not exist — so styling them changed nothing. |
...
**The `view` → `views` migration, stated once — because three documents specify the new shape and none of them says it replaces anything.** The live tree already has this field, in the **singular**: `actions.CORE_KEYS`'s `integrate_edit` row carries `view = "integrate"` (`lua/para-organize/actions.lua:1898`), `keymap_table()` copies it straight through and derives the help group from it (`group = row.view and "Review gate" or "Organize"`), and the pane binder reads a row that has one as "not bound by the ordinary pane loop" (`entry.view == nil and vim.tbl_contains(entry.panes, pane)`). R10 **renames the field to `views` and widens it from a string to a list of view names**, and all three readers move in that one change. There is exactly one live value to carry: `view = "integrate"` becomes **`views = { "integrate" }`** — a one-element list, the same binding, through the same `bind_gate`/`unbind_gate`, with no behaviour change for that row. It takes **no `MOVED_KEYS` entry** and is not a §4.4 deprecation: `view` is an internal row field of a Lua table, never a key a user wrote in `setup()`. Where the spelling *is* user-facing is `actions.register_key{ … views = … }` above, which ships with the list form and never with the singular one. 16 §6 cites this paragraph for the rename; 17 §7 writes its three rows in the same post-migration shape.

**The consolidated `CORE_KEYMAPS` patch, in full.** `CORE_KEYMAPS` (`src/organize_core/config.py`) is the core's reservation list — the `lhs` values a `[[metadata_fields]] keymap` may not claim (07 acceptance 4, split so the *core* owns metadata-vs-core detection). It is written **once, from here**, in a single patch (R9); five documents editing one dict conflict five ways. It is reproduced here in full so a reader of this doc sees the whole reservation without opening four others.

**23 rows exist today**: `<CR> <Esc> <Tab> <S-Tab> s S a m / r p ? A <A-j> <A-k> <C-h> <C-l> <BS> <leader>np <leader>na <leader>nr <leader>mc <leader>mx`.

**Two of them are corrected in place.** The stored value drifted from the Lua action name, and §10.7's gate cannot go green until they agree:

| lhs | value today | corrected to | the row it must match |
|---|---|---|---|
| `S` | `sort` | **`sort_cycle`** | `actions.CORE_KEYS` `name = "sort_cycle"` |
| `<BS>` | `back_to_parent` | **`back`** | `actions.CORE_KEYS` `name = "back"` |

**Thirty rows are added**, by contributing document:

| Doc | lhs → action name | Rows |
|---|---|---|
| 12 | `e` → `integrate_edit`; `<leader>mi` → `integrate`; `<leader>mm` → `integrate_mode` | 3 |
| 15 | `zi` → `cycle_fields` | 1 |
| 16 | `f` → `hint_row`; `F` → `hint_vault`; `.` → `repeat_destination`; `P` → `pin_destination`; `x` → `mark_capture`; `<leader>ba` → `batch_all`; `<leader>bc` → `batch_clear`; `<leader>d` → `mru`; `<leader>f` → `filter_session` | 9 |
| 16 | `1` … `9` → `numeric_accept` — nine rows carrying one name, because the name a reservation reports is the knob a colliding user must go read, and for the digits that knob is the boolean `ui.organize.numeric_accept` (16 §3.4; the §10.7 gate exemption) | 9 |
| 17 | `u` → `undo`; `U` → `redo`; `H` → `history` | 3 |
| 18 | `<C-n>` → `teach_next`; `<C-p>` → `teach_prev`; `<C-g>` → `teach_hint`; `<C-r>` → `teach_replay`; `<C-q>` → `teach_quit` | 5 |

**53 rows after the patch** (23 + 30), and that count is the thing to assert. Two rows a reader expects to find here are **already present and are not part of the patch**: `A` → `auto_organize` (13 §1) and `p` → `toggle_preview`. 16 §3.8 narrows `p` to the organize pane, which is an `actions.lua` change and not a reservation change — the reservation was already correct.

The Lua half of the same patch moves with it, never after it: `ui.DEFAULTS.keymaps.buffer` (from which `config.lua` derives `KEYMAP_NAMES`, the closed set of rebindable names) and `actions.CORE_KEYS`. §10.7's gate compares the two sets, so a one-sided edit is red on arrival. **The nine digits are the one thing that does not cross**: they are `CORE_KEYMAPS` rows only, never `keymaps.buffer` names, and §10.7 names that exemption rather than leaving it to be discovered.
...
7. **Doc-drift gates.** README keymap table ≡ `?` help pane ≡ which-key registrations ≡ `keymap_table()` (one table, every surface generated from it); README/`:help` config table ≡ the schema leaves; **`test_every_subcommand_is_documented`** — documented CLI invocations ≡ the real subcommand/flag set, checked against `README.md` **and** `doc/para-organize.txt` (this is the gate doc 18 §"coverage" defers subcommand coverage to; subcommands are *documentation* coverage, not tutorial coverage). Each gate fails on either direction of drift. Additionally, `CORE_KEYMAPS`'s key set ≡ the set of non-metadata `lhs` values in `keymap_table()`, exported to a fixture by the headless-nvim gate — this doc owns `config.py`, so it owns that drift gate. **It has exactly one exemption, and the gate names it in the gate itself: the nine digits `1`–`9`** (16 §3.4). They are nine `CORE_KEYMAPS` rows, because the collision surface must reserve each digit separately against a `metadata_fields` entry; `keymap_table()` emits **one** synthetic `1-9` help row. The two surfaces therefore differ by nine rows on purpose. The reason the fix is an exemption rather than nine emitted rows: `keymap_table()` is where the closed `keymaps.buffer` name set comes from, so nine rows there would publish nine rebindable action names for what is **one behaviour governed by one boolean** (`ui.organize.numeric_accept`) and cannot be rebound at all — the schema would grow nine keys nobody can meaningfully set, which is 03 §1's dead-key defect arriving through the front door. The gate subtracts those nine literals from both sides and asserts equality on the remainder; **any other difference is a failure**. Written down, an exemption is part of the gate; undocumented, it is a hole in it, which is why this one is stated with its reason and why a second exemption may not be added without amending this sentence.
