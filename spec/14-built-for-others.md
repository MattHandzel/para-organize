...
|---|---|
| `organize <anything>` with no config file | **Refuse.** `ConfigError` naming the expected path plus "`organize health --example-config` prints a complete commented example" — the current `load_config` text (`config.py:1454-1460`) is the standard; "organize-core never runs on implicit defaults" is LAW. |
...
| D6 | `06 §"taskwarrior"`: Fibonacci importance guide with few-shot examples mined from Matt's task export | Prompt few-shots that quote a private corpus are **config-supplied** (`options.examples_file`) with a neutral built-in fallback; one user's task history never ships as code | consumer options | neutralise |
...
| F4 | `ui.DEFAULTS.display.timestamp_format = "%b %d, %I:%M %p"` (12-hour, US) | A default. A stranger sets `%Y-%m-%d %H:%M`; the renderer must not assume a width or a locale | `ui.DEFAULTS` | document |
...
| Where state/config files live | **environment** (`$ORGANIZE_CORE_CONFIG_DIR`, `$ORGANIZE_CORE_STATE_DIR`) only | needed by tests and by packagers, and it redirects nothing but paths |
...
| `[consumers.*] examples_file` (D6) | **core** | Consumers are core-side entirely (10 §1). |
| `ui.display.score_high` / `score_medium` | **nvim** | Purely which highlight group a rendered score gets; the scores themselves cross the wire unchanged, so no CLI counterpart can disagree. |
| `ui.format.*`, header composition, view registry, `actions.register_key`, `on_notify`, `hooks.*` (§5) | **nvim** | All are how the client draws and dispatches; the core has no view of them and none of them can change a vault byte. |

### 4.2 Naming

`snake_case` throughout, both TOML and Lua. Section = subsystem (`[vault]`, `[suggestions]`, `ui.display`). Booleans are named for their ON state and default in the direction that surprises least (`show_scores = true`, never `no_scores`). Durations carry `_ms`. A key ending `_dir`/`_path` holds a path and is `~`-expanded; every other path-shaped value is **relative to the vault root** and refused if absolute (`config.py:957`). **Plural names a collection, singular names a scalar** — the standing PARA vocabulary ruling, applied to all keys. No abbreviations that a reader must decode (`recency_decay`, not `rdecay`).
...
Precedence, always, no exceptions: **explicit argument > config file > shipped default**. A partially-applied config is never left behind — a failed validation rolls back (the `init.setup()` gate ruling).
...
| UI knobs | `setup()` — `ui.display.*`, `ui.float_opts`, `ui.icons`, `ui.win_options`, `ui.capture_pane_keymaps`, `ui.close_on_complete` | STABLE |
| Keymaps | `keymaps.buffer.<name>` by action name; `""` unbinds | STABLE (the **name set** is the contract; adding names is a MINOR) |
...
| Routes / descriptions | `[[routes]]`, `[descriptions]`, `organize describe` | STABLE |
...
| Row formatters | `ui.format.suggestion / entry / capture_line(item, ctx) -> string` callbacks, falling back to the built-in `string.format` | PROVISIONAL — **new seam** |
| Capture-header composition | ordered field list + per-field renderer, replacing the fixed pipeline and literal `HEADER_KEYS` | PROVISIONAL — **new seam**, and the mechanism doc 15's field policy is built on |
...
| Extra actions | `actions.register_key{ name, default, desc, panes, mode, fn }`, which also adds the row to the help pane and the rebindable name set | PROVISIONAL — **new seam** (today a new action needs edits in three files) |
| Score thresholds | `ui.display.score_high / score_medium` (today literals `2.0` / `1.0` in `render.lua`) | PROVISIONAL — **new key** |
| Lifecycle hooks | `state.on(fn)` (exists); `on_notify(level, msg)`; `hooks.before_move / after_move / after_session` — **observational only: a hook cannot veto or alter an operation**, because a client that could veto would make the CLI and the UI disagree about whether a move happened (10 §1) | PROVISIONAL — `state.on` exists, the rest are **new seams** |
...
2. Show three metadata fields in the capture header, in a chosen order, and hide the rest until asked. *(header composition seam)*
3. Add a sixth right-pane view. *(view registry)*
...
| Artifact | Obligation |
|---|---|
| `README.md` | What it is in three sentences; a 60-second quickstart (install core → `organize init --vault` → `setup{}` → `:ParaOrganize start`); install snippets for lazy.nvim and packer; the generated keymap and subcommand tables; a link to `:help`. It is the stranger's only guaranteed read. |
...
1. A test environment with `$HOME` pointed at an empty directory, a fixture vault at `$HOME/vault`, `organize init --vault "$HOME/vault"`, and `setup({})` in headless nvim yields a working session: capture renders, suggestions render, `<CR>` moves the note, original archived — **with no file in the repo naming Matt, his vault, or his hosts having been read**.
2. `grep -R "Obsidian/Main\|matthandzel\|home/matth" src/ lua/ plugin/ deploy/` returns hits only in provenance comments and the labelled case-study example — never in a default value, a hint string, or a shipped config template.
3. Deleting the config file and running any `organize` subcommand exits non-zero with a message containing the config path and `--example-config`; it never proceeds on defaults.
...
12. The origin user's live configuration is reproducible entirely through the public config surface — no key exists that only he can reach.
...
4. **Must-not-be-connected invariants.** (a) No module in `src/` constructs a URL from anything but config — AST gate, extending `test_repo_hygiene.py`'s pattern. (b) With `[consumers]` empty and `llm.ollama_host` unset, no LLM client is ever instantiated. (c) Teach mode, run with the configured vault made read-only *and* watched, performs zero writes and zero reads outside its temporary fixture.
5. **De-Matt-ification is enforced, not remembered.** A repo-wide gate scans `src/`, `lua/`, `plugin/`, `deploy/` for the origin-user string set (`matth`, `matthandzel`, `Obsidian/Main`, `server.matthandzel.com`) and fails on any hit outside an allowlist of provenance comments and the labelled case-study file. The allowlist is the work list of §3 shrinking to zero.
...
7. **Doc-drift gates.** README keymap table ≡ `keymap_table()`; README/`:help` config table ≡ the schema leaves; documented CLI invocations ≡ the real subcommand/flag set. Each gate fails on either direction of drift.
