...
|---|---|---|---|---|
| 1 | `welcome` | what the sandbox is; where it lives; that your real files are not here; the five keys that drive the narrator | teach_next, teach_prev, teach_hint, teach_replay, teach_quit | `ack` |
...
| 22 | `auto-organize` | what the machine will file without you, what it will never touch, and how the trust ladder earns its way up (13 §1) | auto_organize | `action_log`: an auto-applied `move` whose `actor` is not human, with `applied_via` recorded; `vault`: the below-threshold capture was left where it was |
...
- **Keymap coverage stays a lesson obligation.** `test_every_binding_is_taught` requires every row of `actions.keymap_table()` to appear in some lesson's `teaches` list, with `teaches_metadata = true` covering every `metadata_fields` row at once. Explicit, named exemptions only: **`cancel`, `stop`, `teach`** — the first two covered by the `done` lesson in prose, the third being the thing you are already inside. Ship a new binding without a lesson and the suite goes red. ⚠ **The five narrator controls are taught, not exempted**, and the choice is deliberate: `welcome` is the lesson the user must press `teach_next` to leave, so the group is genuinely exercised where it is listed, and the exemption list stays at three. An exemption is a permanent hole in a coverage gate; a lesson that already drives the key is not.
...
Per 10 §3, behavior lives in the core config so CLI and UI cannot disagree; the `setup()` table keeps UI-only concerns. **Core `[teach]`**: `sandbox_dir`, `corpus_seed`, `lessons_path`, `stall_hint_seconds`, `max_generations` — all of them are read by `organize teach --check` with no Neovim in the process, so none can live client-side. **Nvim `ui.teach`**: `narrator`, `advance_delay_ms`, `auto_offer` — a headless CLI has no first run to offer and no float to place. The five narrator bindings are nvim-side too, but they are **not** `ui.teach` keys: they are `keymaps.buffer.teach_next` / `teach_prev` / `teach_hint` / `teach_replay` / `teach_quit`, leaves of the same closed `keymaps.buffer` record as every other action name (§4.3, 16 §6).
